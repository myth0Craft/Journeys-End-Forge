...
public class InvertedShadowBlock extends Block {

	public InvertedShadowBlock(Properties p_312723_) {
		super(p_312723_);
	}
		...
		if (pContext instanceof EntityCollisionContext entityContext) {
			...
			if (entity instanceof LivingEntity living) {
				if (!(living.getMainHandItem().is(ModTags.Items.CAN_PASS_THROUGH_SHADOW_BLOCKS)
						...
						|| living.getInBlockState().is(ModBlocks.ECLIPSED_SHADOW_BLOCK.get()) || living.getType().is(ModTags.Entities.SHADOW_MOBS)) {
					return Shapes.empty();
		...
		if (pContext instanceof EntityCollisionContext entityContext) {
			...
			if (entity instanceof LivingEntity living) {
				if (living instanceof Player player) {
					...
					} else if (living.getInBlockState().is(ModBlocks.ECLIPSED_SHADOW_BLOCK.get())) {
						return Shapes.empty();
					}
				}
			}
